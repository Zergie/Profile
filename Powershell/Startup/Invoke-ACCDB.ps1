...
'@
        ...
        $msIn = New-Object IO.MemoryStream(,$compressedBytes)
        $gzStream = New-Object IO.Compression.GzipStream($msIn, [IO.Compression.CompressionMode]::Decompress)
        ...
        Write-Host "Connecting to Provider=Microsoft.ACE.OLEDB.12.0;Data Source=$Path;Persist Security Info=False;"
        $cn = New-Object -ComObject ADODB.Connection
        $cn.Open("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=$Path;Persist Security Info=False;")
        
        $sql = "CREATE TABLE [$Table] ($($Fields | ForEach-Object { "$($_.Name) $($_.Type)" } | Join-String -Separator ', '))"
        Write-Host $sql
        $cn.Execute($sql) | Out-Null
        
        $Data | ForEach-Object {
            $sql = "INSERT INTO [$Table] ($($Fields | ForEach-Object { $_.Name } | Join-String -Separator ', ')) VALUES ($($_ -split "," | ForEach-Object { "'$($_)'" } | Join-String -Separator ', '))"
            Write-Host $sql
            $cn.Execute($sql) | Out-Null
        }
        ...
        Write-Host "Table $Table created in $Path"
    }
    default {}
}
